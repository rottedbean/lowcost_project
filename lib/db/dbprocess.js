...
async function updateSearchTable(card) {
  queryStr1 = `SELECT * FROM searchtable WHERE code_list LIKE '%${card.pack_code}%'`;
  var result1 = await getDBResult(queryStr1);
  if (result1.length === 0) {
    queryStr2 = `SELECT * FROM searchtable WHERE name = '${card.name}'`;
    var result2 = await getDBResult(queryStr2);
    if (result2.length === 0) {
