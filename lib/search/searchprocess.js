...
async function getsearchresult(keyword) {
  //검색으로는 idxtable에서 줄수 있는 값만, 상세로 넘어가는 부분을 별개로 생성필요
  var return_value;
  ...
  if (!result.length) {
    ...
    for (let i = 0; i < result.length; i++) {
      if (!result[i].idx_list) {
        ...
        //imglist받기는 클라이언트에서 굴려도 되는거 아닐까...
        imglist = await getImageList(result[i].idx_list);
        searchresult_array[i] = {
          cardname: result[i].name,
          lowcost: lowcost,
...
async function todetailpage(cardname) {
...
async function getImageList(idx_array) {
  let imagelocationList = [];
  var nonfilteredlist = idx_array.split(",");
  const list = nonfilteredlist.filter(Boolean);
  for (let i = 0; i < list.length; i++) {
    let queryStr = `SELECT img FROM card_data WHERE idx = '${list[i]}'`;
    let result = await dbmodule.getDBResult(queryStr);
    if (!result) {
      //is null
    } else {
      imagelocationList.push(result);
    }
  }
