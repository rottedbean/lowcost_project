...
function CrawlingHtml(url) {
  GetHtml(url).then((html) => {
    var is_utf_8 = JSON.stringify(html.headers).includes("utf-8");
    if (!is_utf_8) {
    ...
    switch (true) {
      case url.includes("www.tcgshop.co.kr"):
        Case_tcgshop($).then((data) => {
          return data;
        });
        break;
      case url.includes("www.carddc.co.kr"):
        Case_carddc($).then((data) => {
          return data;
        });
        break;
      case url.includes("cardkingdom"):
        Case_cardkingdom($).then((data) => {
          return data;
        });
        break;
      case url.includes("tcgmart"):
        Case_tcgmart($).then((data) => {
          return data;
        });
        break;
    }
...
CrawlingHtml("https://www.carddc.co.kr/product_list.html?k_cateno=116");
CrawlingHtml(
  "https://smartstore.naver.com/cardkingdom/category/090bdc4517cb4468852d3e6370c2d330?cp=1"
);
CrawlingHtml(
